...
class Trainer():
    def __init__(self, model_1, model_2, options, total_client_num, client_num):
...
#         self.privacy_engine_1 = PrivacyEngine()
#         self.privacy_engine_2 = PrivacyEngine()

        os.makedirs(self.save_dir, exist_ok=True)
        if self.sample == 'sliding_window':
            train_logs, train_labels = sliding_window(self.data_dir, self.total_client_num,  self.client_num,
                                                  datatype='train',
                                                  window_size=self.window_size)
            val_logs, val_labels = sliding_window(self.data_dir, self.total_client_num, self.client_num, 
                                              datatype='val',
                                              window_size=self.window_size, 
                                              sample_ratio=0.001)
...
#             self.model_1, self.optimizer_1, self.train_loader = self.privacy_engine_1.make_private(
#                 module=self.model_1,
...
#             self.model_2, self.optimizer_2, self.train_loader = self.privacy_engine_2.make_private(
#                 module=self.model_2,
...
#             )
    ...
    def resume(self, path, load_optimizer=True):
        ...
        self.model_1.load_state_dict(checkpoint['state_dict_1'])  #要記得改state dict
        self.model_2.load_state_dict(checkpoint['state_dict_2'])
        if "optimizer" in checkpoint.keys() and load_optimizer:
            print("Loading optimizer state dict")
            self.optimizer_1.load_state_dict(checkpoint['optimizer_1'])  #看一下checkpoint有沒有這個index
...
#                 loss.backward()  # 這裡能微分的應該是forward函數 但若本身函數沒有變數的話怎麼辦 
                self.optimizer_1.step()
                
                self.optimizer_2.zero_grad() # 想一下這兩行梯度真的要歸0嗎 猜測是LSTM才會這樣
                self.optimizer_1.zero_grad()
    ...
    def start_train(self):
        for epoch in range(self.start_epoch, self.max_epoch):
            if epoch == 0:
                self.optimizer_1.param_groups[0]['lr'] /= 32   # lr的變化或許也是可以探討的一部份
            ...
            if epoch in self.lr_step:
                self.optimizer_1.param_groups[0]['lr'] *= self.lr_decay_ratio
                self.optimizer_2.param_groups[0]['lr'] *= self.lr_decay_ratio
            self.train(epoch)
