...
class Predicter(): 
    ...
    def predict_unsupervised(self):
        ...
        model_2 = self.model_2.to(self.device)
#         model.load_state_dict(torch.load(self.model_path, map_location='cuda:0')['state_dict'])
        model_1.eval()
        model_2.eval()
#         print('model_path: {}'.format(self.model_path))
        test_normal_loader, test_normal_length = generate('hdfs_test_normal', self.total_client_num, self.client_num)
        test_abnormal_loader, test_abnormal_length = generate('hdfs_test_abnormal', self.total_client_num, self.client_num)
        TP = 0
        FP = 0
        # Test the model
        ...
        with torch.no_grad():
            for line in tqdm(test_normal_loader.keys()):
                for i in range(len(line) - self.window_size):
                    ...
                    clientout, clientpreh, clientprec = model_1(features = [seq0[:,:int(self.window_size/2),:], seq1[:,:int(self.window_size/2),:]], device = self.device) # 想一下要怎麼切
                    output = model_2(features=[seq0[:,int(self.window_size/2):,:], seq1[:,int(self.window_size/2):,:]]
        ...
        R = 100 * TP / (TP + FN)
        F1 = 2 * P * R / (P + R)
        Acc = 100 * (TP + test_normal_length - FP) / (test_abnormal_length + test_normal_length) 
        self.f1 = F1
        self.acc = Acc
        self.precision = P
        self.recall = R
        print(
            ...
            .format(FP, FN, P, R, F1))
        print('actual positive (TP+FN): {}, actual negative (FP+TN): {}'.format(test_abnormal_length, test_normal_length))
